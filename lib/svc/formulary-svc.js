...
svc.getProductByName = function(productName){
    ...
    eventHandler.resolveRequest(reslerObj).then(function(response){
        console.log('The product %s was retrieved!', productName);
...
svc.getCoverage = function(productId, planId){
    ...
    eventHandler.resolveRequest(reslerObj).then(function(response){
        if(response instanceof Error){
            ...
            console.log('Plan coverage for the productId: %s and the planId: %s was retrieved!', productId, planId);
...
svc.getFormularyTiers = function(){
    ...
    eventHandler.resolveRequest(reslerObj).then(function(response) {
        console.log('Formulary tiers were retrieved!');
        d.resolve(response);
    }, d.reject);
    return d.promise;
};
...
svc.getFormularyTier = function(tierId){
    ...
    svc.getFormularyTiers().then(function(tiers){
        ...
        console.log('Formulary tier %s was retrieved!', tierId);
        d.resolve(tier);
    }, d.reject);
    return d.promise;
};
